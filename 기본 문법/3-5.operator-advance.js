...
// 배열이 중첩되어 있으면, 해달 배열에 대해서도 분해대입을 할 수 있다. 이 때에는 등호의 좌측에서도 배열이 중첩된 것처럼 써주면 된다.
{
...
// 다음과 같이, 변수의 선언과 동시에 객체의 속성을 해당 변수에 대입할 수 있다.
{
  ...
  console.log(`prop1, prop2:`, prop1, prop2);
}
...
{
  let a, b;
  // 문장이 여는 중괄호(`{`)로 시작되면 이는 `블록`으로 간주되므로, 아래와 같이 분해대입을 할 때는 식 전체를 괄호로 둘러싸주어야 한다.
  ({a, b} = {a: 1, b: 2});
